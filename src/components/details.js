...
class Details extends React.Component {
    ...
    componentDidMount() {
        ...
        axios({
            ...
            .then(respone => {
                this.setState({ restaurants: respone.data.restaurants, restaurantId: restaurant })
            })
            .catch()
    ...
    getData = async (data) => {
        try {
            const response = await fetch(`http://localhost:2021/payment`, {
                method: "POST",
                headers: {
    ...
    handleorders = ()=>{
        const {  email, password, firstname, lastname,phNumber,address,menuItems,subTotal,restaurantId } = this.state;
        const signUpObj = {
            email:email,
            password:password, 
            firstname:firstname,
            ...
            <div>
                <div>
                    ...
                    <div>
                        ...
                        <button className="btn-outline-danger button " onClick={() => this.handleModalState('galleryModalIsOpen', true)} >Click to see Gallery</button>
                    </div>
                    <div className="heading">{restaurants.name}</div>
                    <button className="btn btn-outline-danger btn-order" onClick={this.handleOrder}>Place Online Order</button>
                    <div className="tabs">
                        <div className="tab">
                            <input type="radio" className="btn-check " id="tab-1" name="tab-group-1" checked />
                            <label className="btn btn-outline-danger btn_over me-2" for="tab-1"  >Overview</label>
                ...
                >
                    <div className="fas fa-times close-btnF" onClick={() => this.handleCloseModal('paymentmodalisopen', false)}></div>
                      <div  className="PAYme">
                      <div className="payhead">{restaurants.name}</div>
                      <h3 className="payitem-total">SubTotal :{subTotal}</h3>
                      <h3 className="payitem-total"> {menuItems.name}</h3>
                    <button className="btn btn-danger pay" onClick={this.Payment}> pay </button>
